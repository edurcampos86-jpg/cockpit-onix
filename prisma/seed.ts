...
async function main() {
  ...
  for (const t of templates) {
    await prisma.script.create({
      data: { ...t, isTemplate: true, authorId: admin.id },
    });
  }

  // Criar posts de exemplo para a semana atual
  ...
  for (const p of posts) {
    ...
    const post = await prisma.post.create({
      data: {
        ...
        format: p.format,
        category: p.category,
        status: isPast ? "publicado" : "rascunho",
        scheduledDate: date,
    ...
    for (const t of taskDefs) {
      ...
      await prisma.task.create({
        data: {
          title: t.title,
          type: t.type,
          status: isPast ? "concluida" : "pendente",
          priority: "media",
          dueDate: due,
          assigneeId: t.type === "edicao" ? support.id : admin.id,
          postId: post.id,
        },
      });
    }
