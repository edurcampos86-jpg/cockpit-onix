...
/**
 * Boot do Dia — heurística que sugere as 3 Prioridades do Dia.
 *
 * Regras implementadas, em ordem decrescente de peso:
 *   1. Ação em Q1 (crítico e urgente) atrasada ............ peso 100
 *   2. Ação atrasada em qualquer quadrante ................ peso  80
 *   3. Ação que vence hoje e está marcada importante ...... peso  75
 *   4. Ação que vence hoje ................................ peso  60
 *   5. Cliente classe A sem contato há >= 25 dias ......... peso  70
 *   6. Cliente classe B sem contato há >= 55 dias ......... peso  50
 *
 ...
 *
 * Escreve sempre no dia corrente (hojeBahia) e só em slot vazio, então não
 * existe histórico a recuperar: rodar hoje não preenche dias passados.
 */
...
export async function gerarCandidatos(userId: string): Promise<BootCandidato[]> {
  ...
  // Ações atrasadas ou vencendo hoje.
  for (const a of acoes) {
    const vence = a.vence;
    if (!vence) continue;
    const atrasada = vence < inicioHoje;
    const venceHoje = vence.toISOString().slice(0, 10) === hoje;
  ...
  // Clientes fora da cadência Supernova.
...
/**
 * Preenche os slots VAZIOS do dia corrente com os melhores candidatos.
 * Nunca sobrescreve prioridade que o usuário digitou; marca
 * sugeridaPorBoot=true para o UI conseguir diferenciar.
 */
