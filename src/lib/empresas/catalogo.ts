 ...
 * A mesma lista de empresas estava escrita em três lugares que não se
 * conheciam, e as três já divergiam:
 *
 *   • `scripts/seed-empresas.ts`      — 8 ids (quem vira linha em `Empresa`)
 *   • `src/lib/hub-ecossistema/nos.ts` — outros 8 ids (quem orbita no hub)
 *   • `scripts/reparent-empresas.ts`   — 7 ids (quem pendura na raiz)
 *
 * A interseção dos dois primeiros é 6. `agro` e `contabil` aparecem no hub e
 * não no cadastro; `planejamento` aparece no cadastro e não no hub. Nada
 * quebrava de imediato — por isso a divergência sobreviveu —, mas ela tem
 * consequência silenciosa dos dois lados, e é essa consequência que este
 * arquivo torna visível.
 *
 ...
 *
 * ── QUEM CONSOME ─────────────────────────────────────────────────────────
 *   • `scripts/seed-empresas.ts`      — deriva a lista de `idsCadastradas()`
...
/** A empresa-mãe. Mesmo id em seed, reparenting e `Empresa.id`. */
...
export type EmpresaDoGrupo = {
   ...
   *
   * Consequência de estar `false`: a empresa é INVISÍVEL para o RBAC. Não dá
   * para conceder acesso a ela, e o hub nunca a mostra travada — ausência de
   * cadastro não é negação de acesso (ver `hub-ecossistema/acesso.ts`).
   */
  cadastrada: boolean;
  /**
...
export const EMPRESAS_DO_GRUPO: readonly EmpresaDoGrupo[] = [
  ...
  { id: "educacao", nome: "Onix Educação", cadastrada: true, noHub: true },
  {
    ...
    nota:
      ...
      "existe, embora possa abri-la e tenha acesso concedido a ela. Virar o " +
      "9º nó é decisão de produto pendente (muda o ângulo da órbita de 45° " +
      "para 40°, o que a geometria já suporta: `posicaoOrbital` recebe o " +
      "total). Enquanto não vier, fica declarado aqui em vez de esquecido.",
  },
  {
    id: "agro",
    nome: "Onix Agro",
    ...
    nota:
      ...
      "Por isso o hub tem a guarda `cadastradas` em `acesso.ts:83`: sem ela, " +
