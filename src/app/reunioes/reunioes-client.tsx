...
import { MesaConciliacao } from "@/components/reunioes/mesa-conciliacao";
...
export function ReunioesClient({ mesaHabilitada }: { mesaHabilitada: boolean }) {
  const [meetings, setMeetings] = useState<MeetingData[]>([]);
  const [loading, setLoading] = useState(true);
  // Começa no legado para que a flag OFF preserve inclusive o primeiro paint.
  // A experiência nova só substitui a tela depois que o endpoint confirma a flag.
  const [modoMesa, setModoMesa] = useState<"ativa" | "legado" | "erro">(
    mesaHabilitada ? "ativa" : "legado",
  );
  const [mesa, setMesa] = useState<PlaudConciliacaoPayload | null>(null);
  const [mesaErro, setMesaErro] = useState<string | null>(null);
  ...
  const carregarExperiencia = useCallback(async () => {
    setLoading(true);
    setMesaErro(null);
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(json?.error ?? `Falha ao carregar a mesa (HTTP ${res.status})`);
      }
      const payload = (await res.json()) as PlaudConciliacaoPayload;
      if (!Array.isArray(payload.items) || !payload.metricas || !payload.janela) {
        throw new Error("A mesa devolveu um formato inesperado.");
      }
      setMesa(payload);
      setModoMesa("ativa");
    } catch (erro) {
      setMesaErro(erro instanceof Error ? erro.message : "Não foi possível carregar a mesa.");
      setModoMesa("erro");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (mesaHabilitada) {
      void carregarExperiencia();
      return;
    }
    void fetchMeetings();
  }, [carregarExperiencia, fetchMeetings, mesaHabilitada]);
  ...
  if (modoMesa === "erro") {
    return (
      <div className="p-6 md:p-8">
        <PageHeader
          title="Reuniões Plaud"
          description="Confira o cliente sugerido e leve a transcrição para revisão na ficha antes de salvar."
        />
        <Card className="mt-6 border-destructive/30" aria-live="polite">
          <CardContent className="flex min-h-48 flex-col items-center justify-center gap-3 text-center">
            <p className="font-medium">Não foi possível carregar a mesa.</p>
            <p className="text-sm text-muted-foreground">{mesaErro}</p>
            <button
              type="button"
              onClick={() => void carregarExperiencia()}
