...
export function FloatingChat() {
  ...
  // One conversation per agent — avoids confusing context jumps when navigating between modules.
  ...
  const messages = useMemo(() => {
    if (conversations[agentId]) return conversations[agentId];
    if (agent) return [{ role: "assistant" as const, content: agent.intro }];
    return [];
  }, [conversations, agentId, agent]);
  ...
  const enviar = useCallback(
    async (texto: string) => {
      ...
      const baseMessages =
        conversations[agentId] ?? [{ role: "assistant" as const, content: agent.intro }];

      const novaMsg: Message = { role: "user", content: trimmed };
      const novasMensagens = [...baseMessages, novaMsg];
      setConversations((prev) => ({ ...prev, [agentId]: novasMensagens }));
      setInput("");
      setLoading(true);

      const comAssistente = [...novasMensagens, { role: "assistant" as const, content: "" }];
      setConversations((prev) => ({ ...prev, [agentId]: comAssistente }));

      try {
        const response = await fetch(`/api/agents/${agentId}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ messages: novasMensagens, pathname }),
        });
        ...
        if (!response.ok) {
          ...
          setConversations((prev) => ({
            ...
            [agentId]: [
              ...novasMensagens,
              { role: "assistant", content: `Erro: ${errorText}` },
            ],
          }));
          setLoading(false);
          return;
        ...
        setConversations((prev) => ({
          ...
          [agentId]: [
            ...novasMensagens,
            { role: "assistant", content: `Erro de conexao: ${msg}` },
          ],
        }));
      } finally {
        setLoading(false);
    ...
    [agent, agentId, conversations, loading, pathname]
  );
      ...
      >
        <div className="bg-primary/5 border-b border-border px-4 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground leading-tight">{agent.subtitle}</p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {isInitialState && !loading && (
            <div className="flex flex-col gap-2 pt-2">
              {agent.suggestions.map((sugestao) => (
                <button
                  key={sugestao}
